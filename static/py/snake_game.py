...
THEME_COLORS = {"background":"#171717","food":"#4ade80","snake_head":"#FFD43B","snake_body":"#4B8BBE","snake_eye":"#171717"}
GameState = {"INSTRUCTIONS":"instructions","RUNNING":"running","PAUSED":"paused","GAMEOVER":"gameover"}
TILE_SIZE = 20
...
def set_canvas_size(e=None):
    ...
    container_width = container.clientWidth - (int(style.paddingLeft.replace("px","")) + int(style.paddingRight.replace("px","")))
...
def reset_game(e=None):
    global snake, food, direction, score, game_speed, high_score
    ...
    snake = [{"x":start_x,"y":start_y},{"x":start_x-1,"y":start_y},{"x":start_x-2,"y":start_y}]
    spawn_food()
    direction = {"x":0,"y":0}
    score = 0
    game_speed = 150
...
def update():
    ...
    if head["x"]<0: head["x"]=tile_count_x-1 if head["y"] in (cy1,cy2) else -1
    elif head["x"]>=tile_count_x: head["x"]=0 if head["y"] in (cy1,cy2) else tile_count_x
    if head["y"]<0: head["y"]=tile_count_y-1 if head["x"] in (cx1,cx2) else -1
    elif head["y"]>=tile_count_y: head["y"]=0 if head["x"] in (cx1,cx2) else tile_count_y

    if not (0<=head["x"]<tile_count_x and 0<=head["y"]<tile_count_y) or \
       any(s["x"]==head["x"] and s["y"]==head["y"] for s in snake[1:]):
    ...
    if head["x"] == food["x"] and head["y"] == food["y"]:
        ...
        spawn_food()
        if score%5==0 and game_speed>60: game_speed-=10
    else:
        snake.pop()
...
def draw_tunnels():
    ctx.fillStyle="#27272a";cy1,cy2=(tile_count_y//2)-1,tile_count_y//2;cx1,cx2=(tile_count_x//2)-1,tile_count_x//2
    ctx.fillRect(0,cy1*TILE_SIZE,TILE_SIZE,TILE_SIZE*2);ctx.fillRect((tile_count_x-1)*TILE_SIZE,cy1*TILE_SIZE,TILE_SIZE,TILE_SIZE*2)
    ctx.fillRect(cx1*TILE_SIZE,0,TILE_SIZE*2,TILE_SIZE);ctx.fillRect(cx1*TILE_SIZE,(tile_count_y-1)*TILE_SIZE,TILE_SIZE*2,TILE_SIZE)

def draw_food():
    ctx.fillStyle=THEME_COLORS["food"];ctx.beginPath();ctx.arc(food["x"]*TILE_SIZE+TILE_SIZE/2,food["y"]*TILE_SIZE+TILE_SIZE/2,TILE_SIZE/2.5,0,2*3.14159);ctx.fill()

def draw_snake():
    for i,segment in enumerate(snake):
        ctx.fillStyle=THEME_COLORS["snake_head"] if i==0 else (THEME_COLORS["snake_body"] if i%2!=0 else THEME_COLORS["snake_head"])
        ctx.beginPath();ctx.roundRect(segment["x"]*TILE_SIZE,segment["y"]*TILE_SIZE,TILE_SIZE,TILE_SIZE,[TILE_SIZE/4]);ctx.fill()
    head=snake[0];eye_size=TILE_SIZE/5;ctx.fillStyle=THEME_COLORS["snake_eye"]
    eye_dir=direction if not (direction["x"]==0 and direction["y"]==0) else {"x":1,"y":0}
    if eye_dir["x"]==1:ctx.fillRect(head["x"]*TILE_SIZE+TILE_SIZE/2,head["y"]*TILE_SIZE+eye_size,eye_size,eye_size);ctx.fillRect(head["x"]*TILE_SIZE+TILE_SIZE/2,head["y"]*TILE_SIZE+3*eye_size,eye_size,eye_size)
    elif eye_dir["x"]==-1:ctx.fillRect(head["x"]*TILE_SIZE+eye_size,head["y"]*TILE_SIZE+eye_size,eye_size,eye_size);ctx.fillRect(head["x"]*TILE_SIZE+eye_size,head["y"]*TILE_SIZE+3*eye_size,eye_size,eye_size)
    elif eye_dir["y"]==1:ctx.fillRect(head["x"]*TILE_SIZE+eye_size,head["y"]*TILE_SIZE+TILE_SIZE/2,eye_size,eye_size);ctx.fillRect(head["x"]*TILE_SIZE+3*eye_size,head["y"]*TILE_SIZE+TILE_SIZE/2,eye_size,eye_size)
    elif eye_dir["y"]==-1:ctx.fillRect(head["x"]*TILE_SIZE+eye_size,head["y"]*TILE_SIZE+eye_size,eye_size,eye_size);ctx.fillRect(head["x"]*TILE_SIZE+3*eye_size,head["y"]*TILE_SIZE+eye_size,eye_size,eye_size)

def spawn_food():
    global food
    while True:
        food={"x":random.randint(0,tile_count_x-1),"y":random.randint(0,tile_count_y-1)}
        if not any(s["x"]==food["x"] and s["y"]==food["y"] for s in snake):break

def update_ui(new_state):
    global game_state,score
    game_state=new_state
    for el in[instructions_el,game_over_el,paused_el,ingame_score_display,new_high_score_message_el]:el.classList.add("hidden")
    action_btn_icon.className = "fas fa-play" # Default icon
    if new_state==GameState["INSTRUCTIONS"]: instructions_el.classList.remove("hidden")
    elif new_state==GameState["RUNNING"]:
        ingame_score_display.classList.remove("hidden");score_el.textContent=score
        action_btn_icon.className = "fas fa-pause" # Change to pause icon
    elif new_state==GameState["PAUSED"]: paused_el.classList.remove("hidden")
    elif new_state==GameState["GAMEOVER"]:
        game_over_el.classList.remove("hidden");final_score_el.textContent=score
...
def set_direction(dx, dy):
    ...
    if not (direction["x"] == -dx and direction["y"] == -dy):
        direction = {"x": dx, "y": dy}
...
def handle_action_btn_click(e):
    ...
    if game_state in [GameState["INSTRUCTIONS"], GameState["GAMEOVER"]]:
        start_sound.play(); reset_game(); update_ui(GameState["RUNNING"])
    elif game_state == GameState["RUNNING"]: update_ui(GameState["PAUSED"])
...
def handle_keydown(e):
    ...
    if code=='Space':
        e.preventDefault()
        if game_state==GameState["RUNNING"]: update_ui(GameState["PAUSED"])
        elif game_state==GameState["PAUSED"]: update_ui(GameState["RUNNING"])
        return
